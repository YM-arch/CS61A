...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            if difference > 0:
                ...
                self.stock -= 1
                if difference == 0:
                    return 'Here is your {0}.'.format(self.thing)
                else:
                    return 'Here is your {0} and ${1} change.'.format(self.thing,abs(difference))
    ...
    def restock(self,stock):
        self.stock += stock
        return 'Current {0} stock: {1}'.format(self.thing,self.stock)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        pass_time = Mint.current_year - self.year
        if pass_time >50:
            return self.cents + pass_time - 50
        return self.cents
class Nickel(Coin):
    cents = 5
...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(t,bst_max,num):
        if t.is_leaf():
            return True
        if len(t.branches) == 2:
                if t.branches[0].label <= t.label and t.branches[1].label > t.label:
                    bst_max = t.branches[1].label
                    return all(helper(branch,bst_max,1) for branch in t.branches )
        elif len(t.branches) == 1:
                if num == 0:
                    return all(helper(branch,bst_max,0) for branch in t.branches)
                if t.label < bst_max and t.branches[0].label <=t.label:
                    return all(helper(branch,bst_max,1) for branch in t.branches)
                if t.label >=bst_max and t.branches[0].label >t.label:
                    return all(helper(branch,bst_max,1) for branch in t.branches)
        else:
                return all(helper(branch,bst_max) for branch in t.branches)
        return False
    return helper(t,t.label+1,0)
...
def remove_all(link , value):
    ...
    if link.rest.first == value:
        ...
        remove_all(link, value)     #注意rest指针所指方向
    else:
