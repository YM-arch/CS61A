...
def balanced(m):
    ...
    l , r = left(m) , right(m)
    l_length , r_length = length(l) , length(r)
    l_weight , r_weight = total_weight(end(l)) , total_weight(end(r))
    if l_length * l_weight == r_length * r_weight:
        if is_mobile(end(l)):
            if not balanced(end(l)):
               return False
        if is_mobile(end(r)):
            if not balanced(end(r)):
               return False
        return True
    else:
        return False
...
def replace_leaf(t, find_value, replace_value):
    ...
    "*** YOUR CODE HERE ***"
    # new_t = t
    # for branch in branches(new_t):
    #     if is_leaf(branch):
    #         if label(branch)==find_value:
    #             branch[0] = replace_value
    #     else:
    #         branch = replace_leaf(branch,find_value,replace_value)
    # return new_t
    # the above codes violate the abstraction barriers    (but why?)
    if is_leaf(t):
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    f_t = lambda t:a*t*t+b*t+c
    if -b/(2*a) >=lower_bound(x) and -b/(2*a)<=upper_bound(x):
        if a>0:
            small = f_t(-b/(2*a))
            big = max(f_t(lower_bound(x)),f_t(upper_bound(x)))
            return interval(small,big)
        else:
            big = f_t(-b/(2*a))
            small = min(f_t(lower_bound(x)),f_t(upper_bound(x)))
            return interval(small,big)
    else:
        small = min(f_t(lower_bound(x)),f_t(upper_bound(x)))
        big = max(f_t(lower_bound(x)),f_t(upper_bound(x)))
